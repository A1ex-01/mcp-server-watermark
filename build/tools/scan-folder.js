...
const toolCallbackFn = async (env) => {
    ...
    try {
        ...
        const fullFiles = allFiles.map((file) => path.relative(allowedFolderResolved, file));
        // 过滤出PDF文件
        const pdfFiles = fullFiles.filter((file) => file.toLowerCase().endsWith(".pdf"));
        // 返回PDF文件列表
        return {
            _meta: {},
            content: [
